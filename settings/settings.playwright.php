...
// Suppress deprecation notices during Playwright test setup and execution.
// During cold container compilation, Drupal adds an X-Drupal-Assertion-N HTTP
// header for every error and deprecation (see _drupal_error_header() in
// errors.inc). With many contrib modules, this produces headers totaling >64KB,
// exceeding nginx's fastcgi_buffer_size and causing 502 errors on the first
// request after rebuild.php.
//
...
// preventing both paths from adding assertion headers. This applies during
// both initial setup (PLAYWRIGHT_SETUP) and test child site requests
// (DRUPAL_TEST_IN_CHILD_SITE).
if (getenv('PLAYWRIGHT_SETUP') !== FALSE || (defined('DRUPAL_TEST_IN_CHILD_SITE') && DRUPAL_TEST_IN_CHILD_SITE)) {
